    ...
    DataTypeValue: From<Key>
{
    ...
    pub fn fuzzy_search(
        ...
        if let Some(element) = self.search(item) {
            ...
            let result = {
                ...
                let element = element_ptr.borrow_mut();
            ...
            if should_remove { self.remove(item); } else {
                if let Some(result) = &result {
                    result.0.borrow_mut().decrement_counter(); 
                }
            }
    ...
    pub fn remove_without_weights(&mut self, key: &Key) -> bool {
        ...
        while let Some(node) = &current_node.clone() {
            ...
            if index < node.borrow().size && key.equals(node.borrow().keys[index].as_ref().unwrap()) {
                ...
                if element.borrow().counter == 1 {
                    ...
                    if node.borrow().is_leaf {
                        ...
                        //    as direct neighbors.
                        node.borrow_mut().remove_element(index, self.range());
    ...
    pub fn activate_defining(&mut self, key: &Key, signal: f32) -> Result<f32> {
        ...
        Ok(element.clone().borrow_mut().activate_defining(signal))
    }
